<template>
  <div>
    <div class="filterbox">
      ...
      >
        <el-option
          v-for="(height, index) in heightOptions"
      ...
      >
        <el-option-group>
          <el-option label="打开范围选择器..." :value="0"> </el-option>
        </el-option-group>
        <el-option-group>
          <el-option
      ...
      <el-popconfirm title="确定重置吗？" @confirm="clearFilter">
        <el-button slot="reference" size="mini" plain>重置</el-button>
      </el-popconfirm>
    </div>
    ...
    >
      <div class="datepickerWrap">
        <el-date-picker
          v-model="userDefinedTimeRangeValue"
          type="datetimerange"
        ...
        <div class="granularity">
          ...
          <el-radio v-model="granularityRadio" label="raw">原始数据</el-radio>
          <el-radio
            v-model="granularityRadio"
            label="hour"
            :disabled="hourRadioDisabled"
            >小时平均</el-radio
          >
          <el-radio
            v-model="granularityRadio"
            label="day"
            :disabled="dayRadioDisabled"
            >日平均</el-radio
          >
        </div>
      </div>
    ...
    <!-- 遍历的风速-时间图 -->
    <div v-for="(options, index) in vforList" :key="index">
      <el-card
        class="card"
        shadow="never"
        v-loading="options.loading"
        v-show="
          (siteValue.indexOf(options.siteValue) !== -1 ||
            siteValue.length === 0) &&
          (heightValue.indexOf(options.heightValue) !== -1 ||
            heightValue.length === 0)
        "
      >
        <div class="title">
          <p class="titleleft">
            {{ options.siteLabel }}测风塔 {{ options.heightLabel }}数据
          </p>
      ...
      this.vforList.forEach((item) => {
        ...
        post4SpeedTimeData(data)
          .then((res) => {
            ...
            this.echartsList[item.siteValue][item.heightValue].setOption(
              option
            );
            item.loading = false;
          })
